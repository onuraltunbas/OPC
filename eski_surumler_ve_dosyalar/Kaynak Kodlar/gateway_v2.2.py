...
class KurulumPenceresi(QtWidgets.QDialog):
    ...
    def python_indir_ve_kur(self):
        ...
        def download():
            ...
            try:
                ...
                self.log_yaz("⚠️ Lütfen bekleyin, ekrana pencere gelmeyecek...")
        ...
        def islem():
            ...
            try:
                ...
                if p.returncode == 0:
                    ...
                    lbl_widget.setText("⚠️ Tamamlandı (Uyarı)")
    ...
    def kutuphaneleri_kur(self):
        # BUG FIX: Önce setuptools ve wheel kurulur, sonra asıl kütüphaneler kurulur!
        komut = r'"C:\Python313_32\python.exe" -m pip install --upgrade pip setuptools wheel && "C:\Python313_32\python.exe" -m pip install OpenOPC-Python3x asyncua pywin32 pyro4'
...
# =====================================================================
# 3. BÖLÜM: GATEWAY MOTORU (SIFIR DEĞER FİXİ VE MATRIX EKRANI)
# =====================================================================
class GatewayWorker(QThread):
    ...
    def run(self):
        try:
                    ...
                    while self.is_running:
                        for et_adi in nodes.keys():
                            try:
                                ...
                                # BUG FIX: Kalite kontrolünü esnetip veriyi doğrudan çekiyoruz
                                if sonuc and isinstance(sonuc, tuple) and len(sonuc) >= 2:
                                    val = sonuc[0] 
                                    if val is not None:
                                        await nodes[et_adi].write_value(float(val), ua.VariantType.Double)
                                        # Matrix tarzı konsol akışı
                                        print(f"🟢 {et_adi} -> Değer: {val:.2f}") 
                                    else:
                                        print(f"⚠️ {et_adi} değeri BOŞ geldi.")
                                else:
                                    print(f"⚠️ {et_adi} okunamadı, dönen cevap: {sonuc}")
                            except Exception as alt_hata:
                                print(f"❌ Okuma Hatası ({et_adi}): {alt_hata}")
                                
                        print("-" * 50) # Saniyeleri ayırır
            ...
            print("❌ Kütüphaneler eksik! Lütfen 'Otomatik Kurulum' menüsünden 1 ve 2. butonlara basın.")
        except Exception as e: 
