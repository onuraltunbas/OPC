...
# =====================================================================
# 1. BÖLÜM: SENİN TASARIMIN (Qt Designer Çıktısı)
# =====================================================================
class Ui_btn_sunucu_tara(object):
    def setupUi(self, btn_sunucu_tara):
        ...
        self.cb_sunucu.setGeometry(QtCore.QRect(250, 220, 140, 31)) # İsmi sığsın diye hafif genişletildi
        ...
        self.txt_konsol.setGeometry(QtCore.QRect(30, 340, 740, 201)) # Konsol rahat okunsun diye genişletildi
...
# =====================================================================
# 3. BÖLÜM: GATEWAY MOTORU (SYNC=TRUE UYGULANDI)
# =====================================================================
            ...
            async def main_loop():
                ...
                async with srv:
                    ...
                    while self.is_running:
                        # sync=True belalı timeout hatasını kökten çözer
                        data = opc.read(list(nodes.keys()), sync=True)
                        for name, val, qual, time in data:
                            if val is not None and qual == 'Good':
                                await nodes[name].write_value(float(val), ua.VariantType.Double)
                        print(f"⏳ Akış Aktif: Veriler Başarıyla İletiliyor...")
...
class GatewayApp(QtWidgets.QMainWindow, Ui_btn_sunucu_tara):
    def __init__(self):
        ...
        self.txt_konsol.setStyleSheet("background: black; color: #00ff00; font-family: Consolas;")
    ...
    def baslat(self):
        ...
        if not tags:
            return QtWidgets.QMessageBox.warning(self, "Hata", "Listeden okumak istediğiniz etiketleri işaretleyin (Mavi olsun)!")
    ...
    def durdur(self):
        if self.worker:
            ...
            self.btn_durdur.setEnabled(False)
            print("\n🛑 Gateway başarıyla durduruldu.")

# EXE'nin kendi kendini klonlayıp çökmesini önleyen en kritik satır!
