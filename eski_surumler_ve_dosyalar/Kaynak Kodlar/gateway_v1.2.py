...
class Ui_btn_sunucu_tara(object):
    def setupUi(self, btn_sunucu_tara):
        ...
        self.btn_kurulum_ac.setGeometry(QtCore.QRect(20, 40, 140, 31))
...
class KurulumPenceresi(QtWidgets.QDialog):
    def __init__(self):
        ...
        if sys.maxsize > 2**32:
            ...
            self.lbl_mimari.setText("✅ Python 32-Bit Tespit Edildi.")
    ...
    def python_indir_ve_kur(self):
        def islem():
            ...
            self.log_ekle("📥 Python 3.13 (32-Bit) indiriliyor... (Yaklaşık 25MB)")
            ...
            try:
                ...
                self.log_ekle("⚠️ DİKKAT: En alttaki 'Add Python.exe to PATH' kutucuğunu İŞARETLEYİN!")
    ...
    def run(self):
        try:
            ...
            async def asenkron_gateway():
                ...
                ua_server.set_server_name("Gateway")
                idx = await ua_server.register_namespace("http://gateway/monitoring")
            ...
            print("❌ HATA: Kütüphaneler eksik! Lütfen 'Kurulum' menüsünden kurulumları yapın.\n")
        except Exception as e: 
            print(f"HATA: {e}\n")
        finally: 
...
# =====================================================================
# 4. BÖLÜM: ANA UYGULAMA (Arayüz Bağlantıları ve Derin Tarama)
# =====================================================================
class GatewayApp(QtWidgets.QMainWindow, Ui_btn_sunucu_tara):
    ...
    def tara_sunucu(self):
        try:
            ...
            print("✅ Sunucular taranarak listeye eklendi.\n")
        except ImportError: 
    ...
    def tara_etiket(self):
        try:
            ...
            print("⏳ Klasörlerin içine giriliyor, sensörler aranıyor...\n")

            # 1. Yöntem: Derin Tarama (Klasörleri atla, içindeki yaprakları getir)
            gercek_etiketler = opc.list('**', flat=True)

            # 2. Yöntem: Manuel B Planı (Sanal Makine engeline karşı)
            if not gercek_etiketler or len(gercek_etiketler) <= 5:
                gercek_etiketler = []
                for klasor in opc.list():
                    for alt_klasor in opc.list(klasor):
                        sensorler = opc.list(f"{klasor}.{alt_klasor}.*", flat=True)
                        if sensorler:
                            gercek_etiketler.extend(sensorler)

            # 3. Yöntem: Matrikon Acil Durum Listesi
            if not gercek_etiketler:
                gercek_etiketler = ['Random.Real8', 'Random.Int4', 'Random.Boolean', 'Triangle Waves.Real8', 'Saw-toothed Waves.Int4']

            self.list_etiket.addItems(gercek_etiketler)
            opc.close()
            print(f"✅ {len(gercek_etiketler)} adet etiket başarıyla bulundu ve listelendi!\n")

        except ImportError:
            print("❌ Lütfen '⚙️ Sistem Kurulumları' menüsünden kütüphaneleri yükleyin.\n")
        except Exception as e: 
    ...
    def durdur(self):
        if self.gateway_worker: 
            ...
            self.btn_durdur.setEnabled(False)
            print("\n🛑 Kapatma isteği gönderildi, döngünün bitmesi bekleniyor...")
