...
# YENİ NESİL ÇÖZÜM: EXE'nin yeni kurulan kütüphaneleri anında (kapat-aç yapmadan) görmesini sağlar
...
class Ui_btn_sunucu_tara(object):
    ...
    def retranslateUi(self, btn_sunucu_tara):
        ...
        btn_sunucu_tara.setWindowTitle(_translate("btn_sunucu_tara", "KATOT IIoT Gateway - Tam Otomatik"))
        ...
        self.btn_kurulum_ac.setText(_translate("btn_sunucu_tara", "⚙️ Otomatik Kurulum"))

# =====================================================================
# 2. BÖLÜM: TAM OTOMATİK KURULUM PENCERESİ (MS STORE BYPASS)
# =====================================================================
class KurulumPenceresi(QtWidgets.QDialog):
    ...
    def python_indir_ve_kur(self):
        ...
        def download():
            ...
            self.log_yaz("📥 Python 3.13 indiriliyor... (Bu işlem internet hızınıza göre sürebilir)")
            try:
                ssl._create_default_https_context = ssl._create_unverified_context
                urllib.request.urlretrieve(url, path)
                self.log_yaz("✅ İndirme bitti! Arka planda SESSİZ kurulum başlatılıyor...")
                self.log_yaz("⚠️ Lütfen bekleyin, ekrana hiçbir şey gelmeyecek, sistem kendi kuracak...")
                
                # SESSİZ KURULUM KOMUTU (Ekrana pencere çıkartmaz, C:\Python313_32 klasörüne zorla kurar)
                ...
                self.log_yaz("✅ MÜKEMMEL! Python başarıyla C:\\Python313_32 klasörüne kuruldu.")
                self.log_yaz("👉 Şimdi doğrudan 1. ve 2. butonlara basabilirsiniz.")
                
                # Dinamik Path Eklemesi (Kapat-aç yapmamak için)
    ...
    def kutuphaneleri_kur(self):
        # MS Store'u bypass etmek için Absolute Path (Tam Konum) kullanıyoruz!
        komut = r'"C:\Python313_32\python.exe" -m pip install OpenOPC-Python3x asyncua pywin32 pyro4'
        self.komut_calistir(komut, self.lbl_durum1)

    def dll_kaydet(self):
        komut = r'"C:\Python313_32\python.exe" -m pywin32_postinstall -install'
...
class GatewayWorker(QThread):
    ...
    def run(self):
        try:
            # Eksik kütüphanelerin anında görülmesi için Path garantisi
            hedef = r"C:\Python313_32\Lib\site-packages"
...
# =====================================================================
# 4. BÖLÜM: ANA UYGULAMA (KAPAT-AÇ BYPASSLI)
# =====================================================================
